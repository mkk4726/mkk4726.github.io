...
**이진 처치 R-loss**:
```
L(τ) = E[(Y - μ₀(X) - τ(X)(T - π(X)))²]
```

**연속형 처치 R-loss**:
```
L(τ) = E[(Y - μ₀(X) - τ(X,T)(T - π(X)))²]
```
...
**예시**:
```
행렬 A의 고유값: λ₁ = 10, λ₂ = 0.001, λ₃ = 0.0001

- λ₁ = 10: 이 방향은 정보가 풍부
- λ₂ = 0.001: 이 방향은 정보가 거의 없음
- λ₃ = 0.0001: 이 방향은 거의 정보 없음
```
...
τ(x,t) = τ₀(x,t) + ε(x,t)
```
여기서 ε(x,t)는 R-loss를 거의 변화시키지 않는 함수
...
```
원래 행렬 A의 고유값: [10, 0.1, 0.01, 0.001, ...]
정규화 후 (λ=1): [11, 1.1, 1.01, 1.001, ...]
```
