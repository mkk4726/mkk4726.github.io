...
Done: false
---
- 프로젝트 개요 : [[Career/Project/LensSizing/프로젝트 설명|프로젝트 설명]]
...
**"데이터가 부족해서 정확한 예측이 불가능하다면, AI는 무엇을 할 수 있을까요?"**

저는 억지로 값을 맞추는 대신, **'신뢰할 수 있는 구간'을 제공하여 의사의 의사결정을 돕는 것**이 해답임을 증명했습니다.

렌즈 삽입술(ICL)에서 가장 중요한 안전 지표는 **Vault(렌즈 후면과 수정체 전면 사이의 거리)**입니다. 이 값이 안전 범위에 있어야 백내장이나 녹내장 같은 부작용을 막을 수 있습니다. 하지만 수술 전 데이터만으로 수술 후의 Vault를 정확히 예측하는 것은 매우 어렵고, 기존에는 전적으로 의사의 '감'에 의존해야 했습니다.

이 프로젝트는 의사의 의사결정을 돕기 위해 Vault 값을 예측하는 AI 서비스를 고도화한 과정입니다.

초기에는 예측 오차(MAE)를 줄이는 데 집중했습니다. 하지만 심층 분석 결과, **렌즈 종류(Model Name)**가 Vault 값에 결정적인 영향을 미친다는 사실을 확인했습니다.
문제는 현재 데이터 수집 환경상 이 핵심 정보를 정확히 파악할 수 없다는 점이었습니다. 즉, 모델의 성능 한계는 기술 부족이 아니라, **데이터 자체에 내재된 불확실성(Aleatoric Uncertainty)**에서 기인함을 깨달았습니다.

저는 이 불확실성을 억지로 줄이는 대신, 이를 정량화하여 **"신뢰할 수 있는 범위(Prediction Interval)"를 제공하는 것으로 문제를 재정의**했습니다.
...
이 글은 **ML 시스템 구축부터 데이터 한계 발견, 그리고 문제를 재정의하여 기술적으로 해결하기까지의 치열한 고민과 과정**을 담고 있습니다.
...
여러 모델을 비교하고 튜닝을 진행하는 과정에서 **MAE 110, R² 0.4** 수준에서 성능이 더 이상 오르지 않는 한계에 부딪혔습니다.
데이터 파이프라인을 구축하고, 도메인 지식을 활용해 피처를 개발하고, 하이퍼파라미터를 튜닝하며 집요하게 성능 향상을 시도했지만 역부족이었습니다.

**"왜 성능이 오르지 않을까?"** 라는 질문을 끝까지 파고들었고, 결국 **"렌즈 정보의 부재"**가 원인임을 밝혀냈습니다.
수소문 끝에 렌즈 제조사에 문의한 결과, **렌즈 종류별로 기본 Vault(Base Vault)가 다르다**는 사실을 확인했습니다. 즉, 어떤 렌즈를 썼는지에 대한 정보 없이는 정확한 예측이 불가능했던 것입니다.
하지만 현재 데이터셋은 렌즈 종류가 수기로 입력되어 결측이나 오기입이 많아, 이를 모델링에 활용하기 어려운 상황이었습니다.

이러한 데이터의 한계를 명확히 인정한 후, 전략을 수정했습니다.
"정확한 값을 맞추는 것(Point Prediction)"이 불가능하다면, **"불확실성을 포함한 예측 구간(Prediction Interval)"을 제공**하여 의사의 의사결정을 돕는 것이 더 현실적이고 유용한 해결책이라고 판단했습니다.
...
### 5.1 추상화된 모델 파이프라인 구축

앞선 문제들을 해결하고, 원하는 신뢰수준(Confidence Level)을 만족하는 예측 구간을 생성하기 위해 **추상화된 모델 파이프라인**을 개발했습니다.

#### 기술적 접근: Quantile Regression & CQR

예측 구간을 생성하기 위해 **Quantile Regression**을 기본 모델로 사용했습니다.
일반적인 MSE(Mean Squared Error) 대신, 각 분위수(Quantile)를 추정할 수 있는 Loss Function을 사용하여 모델을 학습시켰습니다. 특히 이상치(Outlier)에 강건한 모델을 만들기 위해 **Huber Loss**를 기반으로 한 Quantile Loss를 적용했습니다.
